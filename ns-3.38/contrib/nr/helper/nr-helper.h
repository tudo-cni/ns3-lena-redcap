...
class BwpManagerGnb;
class BwpManagerUe;
class NrMacSchedulerRessourceManager;
...
class NrHelper : public Object
{
  public:
    ...
    NetDeviceContainer InstallUeDevice(
        const NodeContainer& c,
        const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>>& allBwps,
        bool sdt, NrChip chip,uint8_t numberOfPanels = 1);

    NetDeviceContainer InstallRedCapUeDevice(const NodeContainer& c,
        const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>>& allBwps,
       bool sdt, NrChip chip,uint8_t numberOfPanels = 1);
    /**
    ...
    NetDeviceContainer InstallGnbDevice(
        const NodeContainer& c,
        const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>> allBwps,
        uint8_t numberOfPanels = 1, NrMacSchedulerRessourceManager* schedmanagerPtr = nullptr);
    ...
    void SetUeRedCapAntennaAttribute(const std::string& n, const AttributeValue& v);
  ...
  private:
    ...
    Ptr<NrUePhy> CreateUePhy(const Ptr<Node>& n,
                             ...
                             uint8_t numberOfPanels, bool RedCap);

    Ptr<NetDevice> InstallSingleUeDevice(
        const Ptr<Node>& n,
        const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>> allBwps,
        uint8_t numberOfPanels, bool sdt, bool redcap, NrChip chip);
        
    Ptr<NetDevice> InstallSingleGnbDevice(
        const Ptr<Node>& n,
        const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>> allBwps,
        uint8_t numberOfPanels,  NrMacSchedulerRessourceManager* schedmanagerPtr = nullptr);
    ...
    ObjectFactory m_ueRedCapAntennaFactory;               //!< UE antenna factory
