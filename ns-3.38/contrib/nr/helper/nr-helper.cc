...
NrHelper::NrHelper()
{
    ...
    m_ueRedCapAntennaFactory.SetTypeId(UniformPlanarArray::GetTypeId());
...
static void
InitRural(ObjectFactory* pathlossModelFactory, ObjectFactory* channelConditionModelFactory)
{
    pathlossModelFactory->SetTypeId(WinnerPlusPropagationLossModel::GetTypeId());
    channelConditionModelFactory->SetTypeId(NeverLosChannelConditionModel::GetTypeId());
}

static void
InitSubUrban(ObjectFactory* pathlossModelFactory, ObjectFactory* channelConditionModelFactory)
{
    pathlossModelFactory->SetTypeId(WinnerPlusPropagationLossModel::GetTypeId());
    channelConditionModelFactory->SetTypeId(NeverLosChannelConditionModel::GetTypeId());
}
...
static void
InitUrban(ObjectFactory* pathlossModelFactory, ObjectFactory* channelConditionModelFactory)
{
    pathlossModelFactory->SetTypeId(WinnerPlusPropagationLossModel::GetTypeId());
    channelConditionModelFactory->SetTypeId(NeverLosChannelConditionModel::GetTypeId());
}
...
static void
InitHighDensity(ObjectFactory* pathlossModelFactory, ObjectFactory* channelConditionModelFactory)
{
    pathlossModelFactory->SetTypeId(WinnerPlusPropagationLossModel::GetTypeId());
    channelConditionModelFactory->SetTypeId(NeverLosChannelConditionModel::GetTypeId());
}
...
NrHelper::InitializeOperationBand(OperationBandInfo* band, uint8_t flags)
{
    ...
    static std::unordered_map<BandwidthPartInfo::Scenario, InitPathLossFn, std::hash<int>>
        initLookupTable{
            ...
            {BandwidthPartInfo::Rural,
            std::bind(&InitRural, std::placeholders::_1, std::placeholders::_2)},
            {BandwidthPartInfo::SubUrban,
            std::bind(&InitSubUrban, std::placeholders::_1, std::placeholders::_2)},
            {BandwidthPartInfo::Urban,
            std::bind(&InitUrban, std::placeholders::_1, std::placeholders::_2)},
            {BandwidthPartInfo::HighDensity,
            std::bind(&InitHighDensity, std::placeholders::_1, std::placeholders::_2)},
...
{
    //this function uses default parameters for the used chip with its configurations. 
    NS_LOG_FUNCTION(this);
    Initialize(); // Run DoInitialize (), if necessary
    NetDeviceContainer devices;
    NrChip chip_default = RM520N(3750e6,23);
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i)
    {   
        Ptr<Node> node = *i;
        Ptr<NetDevice> device = InstallSingleUeDevice(node, allBwps, numberOfStreams, false,false,chip_default);
...
NrHelper::InstallUeDevice(const NodeContainer& c,
                          const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>>& allBwps,
                          bool sdt, NrChip chip ,uint8_t numberOfStreams)
{
    NS_LOG_FUNCTION(this);
    Initialize(); // Run DoInitialize (), if necessary
    NetDeviceContainer devices;
    ...
    {   
        Ptr<Node> node = *i;
        Ptr<NetDevice> device = InstallSingleUeDevice(node, allBwps, numberOfStreams, sdt,false,chip);
        device->SetAddress(Mac48Address::Allocate());
        devices.Add(device);
    }
    return devices;
...
NetDeviceContainer
NrHelper::InstallRedCapUeDevice(const NodeContainer& c,
                          const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>>& allBwps,
                          bool sdt, NrChip chip,uint8_t numberOfStreams)
{
    NS_LOG_FUNCTION(this);
    ...
    {
        Ptr<Node> node = *i;
        Ptr<NetDevice> device = InstallSingleUeDevice(node, allBwps, numberOfStreams,sdt,true,chip);
        device->SetAddress(Mac48Address::Allocate());
        devices.Add(device);
    }
    return devices;
...
NrHelper::InstallGnbDevice(const NodeContainer& c,
                           const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>> allBwps,
                           uint8_t numberOfStreams, NrMacSchedulerRessourceManager* schedmanagerPtr)
{
    NS_LOG_FUNCTION(this);
    Initialize(); // Run DoInitialize (), if necessary
    NetDeviceContainer devices;
    ...
    {
        Ptr<Node> node = *i;
        Ptr<NetDevice> device = InstallSingleGnbDevice(node, allBwps, numberOfStreams,schedmanagerPtr);
        device->SetAddress(Mac48Address::Allocate());
        devices.Add(device);
    }
    return devices;
...
NrHelper::CreateUePhy(const Ptr<Node>& n,
                      ...
                      uint8_t numberOfStreams, bool redCap)
{
    NS_LOG_FUNCTION(this);
    ...
    // set device
    phy->SetDevice(dev);
    phy->SetCoresetSymbols(bwp->m_coresetSymbols);
    ...
    for (uint8_t streamIndex = 0; streamIndex < numberOfStreams; streamIndex++)
    {
        ...
        if(redCap)
        {
            antenna = m_ueRedCapAntennaFactory.Create<UniformPlanarArray>(); // Create antenna per stream
            
        }
        else{
            antenna = m_ueAntennaFactory.Create<UniformPlanarArray>(); // Create antenna per stream
        }
...
NrHelper::InstallSingleUeDevice(
    const Ptr<Node>& n,
    const std::vector<std::reference_wrapper<BandwidthPartInfoPtr>> allBwps,
    uint8_t numberOfStreams, bool sdt, bool redcap,NrChip chip)
{
    NS_LOG_FUNCTION(this);
    ...
    for (uint32_t bwpId = 0; bwpId < allBwps.size(); ++bwpId)
    {
        ...
        if(redcap && bwInKhz/100 >200) //bwp is larger than 20Mhz
        {
            bwInKhz =0; 
        }
        ...
        cc->SetDlEarfcn(650000); // hardcoded and not needed
        cc->SetUlEarfcn(650000); // hardcoded and not needed
        ...
        if (bwpId == 0)
        {
            ...
            phy->SetActiveBwpStatus(true);
            mac->SetActiveBwpStatus(true);
        }
        else
        {
        ...
        mac->SetPowerStateChangedCallback(MakeCallback(&NrUeNetDevice::PowerStateChanged,dev));
    ...
    rrc->SetPowerStateChangedCallback(MakeCallback(&NrUeNetDevice::PowerStateChanged,dev));
    ...
    uint8_t numberOfStreams, NrMacSchedulerRessourceManager* schedmanagerPtr)
{
    ...
    Ptr<NrGnbRrc> rrc = CreateObject<NrGnbRrc>(); //defined here for to set a callback to the scheduler
    ...
    for (uint32_t bwpId = 0; bwpId < allBwps.size(); ++bwpId)
    {
        ...
        cc->SetDlEarfcn(650000); 
        cc->SetUlEarfcn(650000); 
        ...
        //sets the manager to control overlapping ressources
        DynamicCast<NrMacSchedulerNs3>(sched)->SetManager(schedmanagerPtr);

        rrc->SetSearchSpaceCreatedCallback(MakeCallback(&NrMacSchedulerRessourceManager::CreateSearchSpace,schedmanagerPtr));
        rrc->SetUpdateRntiMapCallback(MakeCallback(&NrMacSchedulerRessourceManager::UpdateRntiMap,schedmanagerPtr));
        mac->SetPrachOccasionUsedCallback(MakeCallback(&NrMacSchedulerRessourceManager::UpdatePrachRessources,schedmanagerPtr));
    ...
    {
        Ptr<NrGnbRrcProtocolReal> rrcProtocol = CreateObject<NrGnbRrcProtocolReal>();
        rrcProtocol->SetNrGnbRrcSapProvider(rrc->GetNrGnbRrcSapProvider());
        rrc->SetNrGnbRrcSapUser(rrcProtocol->GetNrGnbRrcSapUser());
        rrc->AggregateObject(rrcProtocol);
    }
    ...
    {
        it->second->GetPhy()->SetEnbCphySapUser(rrc->GetNrGnbCphySapUser(it->first));
        rrc->SetNrGnbCphySapProvider(it->second->GetPhy()->GetEnbCphySapProvider(), it->first);
...
void
NrHelper::SetUeRedCapAntennaAttribute(const std::string& n, const AttributeValue& v)
{
    NS_LOG_FUNCTION(this);
    m_ueRedCapAntennaFactory.Set(n, v);
}
...
NrDrbActivator::ActivateDrb(uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
    ...
    if ((!m_active) && (imsi == m_imsi))
    {
        ...
        Ptr<NrGnbRrc> gnbRrc = enbLteDevice->GetObject<NrGnbNetDevice>()->GetRrc();
        NS_ASSERT(ueRrc->GetCellId() == enbLteDevice->GetCellId());
        Ptr<UeManagerNr> ueManager = gnbRrc->GetUeManager(rnti);
        NS_ASSERT(ueManager->GetState() == UeManagerNr::CONNECTED_NORMALLY ||
                  ueManager->GetState() == UeManagerNr::CONNECTION_RECONFIGURATION);
