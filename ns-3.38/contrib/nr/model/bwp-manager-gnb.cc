...
uint8_t
BwpManagerGnb::GetActiveBwp(uint16_t rnti)
{
    auto it = m_activeBwpMap.find(rnti);
    if( it != m_activeBwpMap.end())
    {
        return it->second;
    }
    else{
...
void 
BwpManagerGnb::UpdateActiveBwpMap(uint16_t rnti, uint8_t bwpID)
{
    auto it = m_activeBwpMap.find(rnti);
    if( it != m_activeBwpMap.end())
    {
        m_activeBwpMap.insert(std::make_pair(rnti,bwpID));
    }
    else{
        it->second = bwpID;
    }
...
BwpManagerGnb::DoReportBufferStatus(LteMacSapProvider::ReportBufferStatusParameters params)
{
    ...
    uint8_t bwpIndex;
    bwpIndex = GetActiveBwp(params.rnti);
