      ...
      m_n1Delay(4)
{
    ...
    m_enbCphySapProvider = new MemberNrGnbCphySapProvider<NrGnbPhy>(this);
}
...
NrGnbPhy::SetTddPattern(const std::vector<LteNrTddSlotType>& pattern)
{
    ...
    m_phySapProvider->SetUlSchedDeviation(&m_generateUl);
}
...
void
NrGnbPhy::SetEnbCphySapUser(NrGnbCphySapUser* s)
{
    NS_LOG_FUNCTION(this);
    m_enbCphySapUser = s;
}
...
NrGnbPhy::GetBeamConfId(uint16_t rnti) const
{
    ...
    {
        // rnti of a removed ue is reset to 0. This function does not find the ue and returns the default BeamConfID
        // which is different. Therefore a second beam is schedueled which is not supposed to.
        // Has to be fixed when using multiple beams
...
NrGnbPhy::StartSlot(const SfnSf& startSlot)
{
    ...
    if (m_isPrimary)
    {
        if (m_currentSlot.GetSlot() == 0)
        {
            ...
            {
                auto dci = m_phySapUser->GetDlCtrlDci();
                VarTtiAllocInfo dlCtrlSlot(dci);
...
NrGnbPhy::UlData(const std::shared_ptr<DciInfoElementTdma>& dci)
{
    ...
    //found can be false after transmission of RrcRelease. An uplink msg can be received after deleting list entrys. 
    if(!found)
    {
        return MilliSeconds(0); //we do not receive for this allocation
    }
...
NrGnbPhy::StartVarTti(const std::shared_ptr<DciInfoElementTdma>& dci)
{
    ...
    if(varTtiPeriod != MilliSeconds(0))
    {
                           ...
                           const uint8_t& streamId)
{
    ...
    // beamforming is currently not used and therefore commented out
...
NrGnbPhy::PhyCtrlMessagesReceived(const Ptr<NrControlMessage>& msg)
{
    ...
    else if (msg->GetMessageType() == NrControlMessage::RACH_PREAMBLE)
    {
        ...
        m_phySapUser->ReceiveRachPreamble(rachPreamble->GetRapId(), rachPreamble->GetOccasion(), rachPreamble->GetImsi(), rachPreamble->GetPrachNumber());
    }
...
NrGnbPhy::DoSetSystemInformationBlockType1(LteRrcSap::SystemInformationBlockType1 sib1)
{
    NS_LOG_FUNCTION(this);
    m_sib1 = sib1;
    //Inform Mac about the prachConfig
    m_phySapUser->SetPrachConfigs(NrPhy::GetPrachConfig(m_sib1.servingCellConfigCommon.uplinkConfigCommon.rach_ConfigCommon.rachConfigGeneric.prachConfigurationIndex));
}
...
NrGnbPhy::ReportUlHarqFeedback(const UlHarqInfo& mes)
{
    ...
    if (m_ueAttachedRnti.find(mes.m_rnti) != m_ueAttachedRnti.end())
    {
        ...
        if(!mes.IsReceivedOk())
        {
            std::cout<<"test harq not ok"<< std::endl;
        }
    }
}
