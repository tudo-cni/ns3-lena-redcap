...
class NrMacSchedulerOfdma : public NrMacSchedulerTdma
{
  ...
  protected:
    ...
    BeamSymbolMap AssignDLRBG_manager(uint32_t symAvail, const ActiveUeMap& activeDl, const SfnSf& dlSfn) override;
    BeamSymbolMap AssignULRBG(uint32_t symAvail, const ActiveUeMap& activeUl) const override;
    BeamSymbolMap AssignULRBG_manager(uint32_t symAvail, const ActiveUeMap& activeUl, const SfnSf& ulSfn) override;
    ...
    uint16_t schedStartUe{0};

  private:
    TracedValue<uint32_t> m_tracedValueSymPerBeam;
    static inline SfnSf lastProcessedSlot;
