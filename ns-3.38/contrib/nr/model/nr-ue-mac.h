...
class NrUeMac : public Object
{
  ...
  bool m_drxConfigured{false};
  uint m_drx;
  bool m_edrxConfigured{false};
  uint m_edrx;
  protected:
    /**
  ...
  private:
    ...
    void DoTransmitMsg3(LteMacSapProvider::TransmitPduParameters params);
    ...
    std::unordered_map<uint8_t, LteMacSapProvider::ReportBufferStatusParameters> DoGetBufferStatus();
    ...
    void DoStartContentionBasedRandomAccessProcedure(bool redcap, bool do_2step_sdt);
    ...
    void DciReceptionTimeout();
    void SrSchedulingTimeout();
    void DoSetImsi(uint64_t imsi);
    void DoActivateBwp(LteNrTddSlotType type);
    void DoDeactivateBwp(LteNrTddSlotType type);
    void DoNotifyeDrx(uint frames);
    void DoNotifyDrx(uint frames);
    void DoSetPRnti(uint16_t prnti);

    


    void RandomlySelectAndSendRaPreamble();
    uint16_t ChooseRandomPrachOccasion();
    uint16_t ChooseRandomPrachOccasionThisSlot();
    bool hasPrach(SfnSf sfn);
    ...
    void DoRecvSib1(NrSib1Message sib1, NrPhySapProvider::PrachConfig prachOccasions);
  ...
  private:
    ...
    enum SrBsrMachine : uint8_t
    {
        ...
        WAIT_SCHEDULING //!< Expecting to get ressources for a transmission
    };
    ...
    uint16_t m_raRnti{0};       //!< The RA Rnti
    uint64_t m_imsi{0};        ///< IMSI
    uint8_t m_prachOcc;       //< used Occasion
    ...
    EventId m_waitingForDciReception; //started after receiving RRC_Setup. UE ecpects to receive ressources to transmit RRC_SetupComplete
    EventId m_waitingForSchedulingTimer;


    EventId m_noRaResponseReceivedEvent;
    bool m_waitingForRaResponse{false}; //!< start waiting for a RaResponse after sending a preamble
    ...
    std::tuple<SfnSf,uint8_t> m_msg3Grant;
    NrSib1Message m_sib;
    bool m_sibReceived{false};

    //PRACH parameter
    std::vector<uint8_t> m_prachSfn;
    uint8_t m_prachStartSym;
    uint8_t m_prachDuration;
    uint8_t m_nfX;
    uint8_t m_nfY;
    uint8_t m_numPrachSlots;
    std::string m_preambleFormat;
    uint8_t m_prachOccInSlot;

    bool m_waitForPrachOcc{false};

    bool m_startingRach{false}; 
    uint8_t m_prachSubframe;
    uint8_t m_prachSlot;
    bool m_sib1Received{false};
