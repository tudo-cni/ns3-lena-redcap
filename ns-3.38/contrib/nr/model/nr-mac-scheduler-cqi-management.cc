    ...
    const Ptr<const SpectrumModel>& model) const
{
    ...
    if(ueInfo->m_redcap &&  ueInfo->m_ulMcs > 19 )
    {
        //I use NrEesmIrT2 which enables 256QAM at mcs = 20. RedCap UEs should only use 64QAM
        ueInfo->m_ulMcs = 19;
    }
                                             ...
                                             int8_t maxDlMcs) const
{
    ...
    for (std::size_t stream = 0; stream < info.m_wbCqi.size(); stream++)
    {
        if (info.m_wbCqi.at(stream) > 0)
        {
            ...
            if( ueInfo->m_redcap== true  && mcs > 19) //I use NrEesmIrT2 which enables 256QAM at mcs = 20. RedCap UEs should only use 64QAM
            {
                mcs =19;
            }
