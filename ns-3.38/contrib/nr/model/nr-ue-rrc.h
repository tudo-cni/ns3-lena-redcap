 ...
 */
static const Time UE_MEASUREMENT_REPORT_DELAY_NR = MicroSeconds(1);
...
class NrUeRrc : public Object
{
    ...
    /// allow LtePdcpSpecificLtePdcpSapUser<NrUeRrc> class friend access
    friend class LtePdcpSpecificLtePdcpSapUser<NrUeRrc>;
    /// allow MemberLteAsSapProvider<NrUeRrc> class friend access
    friend class MemberLteAsSapProvider<NrUeRrc>;
    ...
    /// allow MemberLteUeCcmRrcSapUser<NrUeRrc> class friend access
    friend class MemberLteUeCcmRrcSapUser<NrUeRrc>;
  ...
  void SetSdt(bool state);
  void SetRedCap(bool state);

  void SendStoredPackages();

  void SetPowerStateChangedCallback(Callback<void,NrEnergyModel::PowerState> pscc);
  Callback<void,NrEnergyModel::PowerState> m_powerStateChangedCallback;

  // static std::ofstream m_stateTransitionFile;
  // static std::string m_stateTransitionFileName;
  // static std::string m_resultsFolder; //!< The results folder path
  // void PrintStateTransition(uint64_t imsi, uint16_t cellID, uint16_t rnti , State oldState, State newState);
  ...
  private:
    ...
    void DoNotifyRandomAccessSuccessful(UlGrant_s grant, bool sdt);
    /// Notify random access failed function
    void DoNotifyRandomAccessFailed();

    void RefreshBwpInactivityTimer();
    ...
    /**
     * Part of the RRC protocol. Implement the NrUeRrcSapProvider::RecvRrcSetup
     * interface.
     * \param msg the LteRrcSap::RrcSetup
     */
    void DoRecvRrcSetup(NrRrcSap::RrcSetup msg);

    void SendRrcSetupCompleted(LteRrcSap::RrcConnectionSetupCompleted msg);
     ...
     */
    void DoRecvRrcReconfiguration(NrRrcSap::RrcReconfiguration msg);
    /**
    ...
    /**
     * Part of the RRC protocol. Implement the NrUeRrcSapProvider::RecvRrcRelease
     * interface.
     * \param msg NrRrcSap::RrcRelease
     */
    void DoRecvRrcRelease(NrRrcSap::RrcRelease msg);

    void DoRecvRrcResume(NrRrcSap::RrcResume msg);

    void ProcessRrcRelease(NrRrcSap::RRCRelease_IEs rel);
    /**
     ...
     * configuring this.
     *
     * \sa LteRrcSap::MeasConfig, NrUeRrc::m_varMeasReportList
     */
    ...
    void ApplyRadioBearerConfig(NrRrcSap::RadioBearerConfig rbc);

    void ApplySpCellConfig(NrRrcSap::SpCellConfig spcc);

    Time bwpInactivityToTime(NrRrcSap::Bwp_InactivityTimer bwpInactivity);
    /**
    ...
    //save spCellConfig to Configure it at a later time
    NrRrcSap::SpCellConfig m_savedSpCellConfig;
     ...
     * message.
     */
    TracedCallback<Ptr<NrUeRrc>, std::list<LteRrcSap::SCellToAddMod>> m_sCarrierConfiguredTrace;
    /**
    ...
    TracedCallback<uint16_t,std::string> m_stateTransition;
    ...
    void DoSendRrcConnectionRequest();
  ...
  protected:
    bool m_sdt{false};
    bool m_redCap{false};
    bool m_use_2step_sdt{false};
    bool m_sdtPossible{false};
    bool m_sdtConfigured{false};
    std::vector<Ptr<Packet>> m_packetStored;
    uint16_t m_grantedBytes;
    uint16_t m_resumeIdentity;
