...
class NrMacGeneralCschedSapProvider : public NrMacCschedSapProvider
{
  public:
    ...
    {
       return  m_scheduler->DoGetMcs(rnti);
    }

    void SetMcs(uint16_t rnti, std::tuple<uint8_t, uint8_t> mcsTuple) override
    {
        m_scheduler->DoSetMcs(rnti,mcsTuple);
    }

    void SetSchedulerPrachConfig(NrPhySapProvider::PrachConfig prachConfig) override
    {
        m_scheduler->DoSetSchedulerPrachConfig(prachConfig);
    }
...
class NrMacGeneralSchedSapProvider : public NrMacSchedSapProvider
{
  public:
    ...
    void SchedUlMsg3InfoReq(const SchedUlMsg3InfoReqParameters& params) override
    {
        m_scheduler->DoSchedUlMsg3InfoReq(params);
    }
