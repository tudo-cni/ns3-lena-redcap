...
class NrPhy : public Object
{
  public:
     ...
     * \param Rnti RNTI
     */
    void SendRachPreamble(uint32_t PreambleId, uint32_t Rnti, uint8_t occasion, uint16_t imsi, uint32_t prachNumber);

    void SetUlSchedDeviation(std::map<uint32_t, std::vector<uint32_t>>* generateUlMap);
    ...
    void SetCoresetSymbols(uint8_t coresetSymbols);
    ...
    uint8_t GetULSlotDeviation(SfnSf ulSlot);

  
    bool GetRachStatus();

         /**
     * \return a specific Prach Configuration
     */
    NrPhySapProvider::PrachConfig GetPrachConfig(uint8_t index) const;
  ...
  protected:
    ...
    std::map<uint8_t, uint8_t> m_ulSchedDeviationMap; // Map used by GnB map to schedule msg3 receptions
  ...
  private:
    ...
    uint8_t m_coresetSymbols{0}; 
    bool m_hasCoreset = false;
  ...
  enum NrSpectrumValueHelper::PowerAllocationType m_powerAllocationType{
    ...
    std::vector<NrPhySapProvider::PrachConfig> all_Prachconfigs;
    bool m_inRachProcess{false};
