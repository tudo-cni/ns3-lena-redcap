...
NrUeNetDevice::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrUeNetDevice")
            ...
            .AddAttribute("EnergyModelStartTime",
                          "Starting time for the energy model in ms.",
                          UintegerValue(0),
                          MakeUintegerAccessor(&NrUeNetDevice::m_energyStartTime),
                                    ...
                                    uint8_t sourceBwpId)
{
    ...
    for (const auto& msg : msgList)
    {
        ...
        if(m_ccMap.at(bwpId)->GetPhy()->GetActiveBwpStatus()) //Maybe distinguish for UL/DL. Not needed so far because they are alligned
        {
            m_ccMap.at(bwpId)->GetPhy()->PhyCtrlMessagesReceived(msg);
        }     
        else{
            NS_LOG_DEBUG("Discarded CtrlMsg for imsi: "<<m_imsi << " in bwpId: "<< bwpId);
        }               
...
void
NrUeNetDevice::PowerStateChanged(NrEnergyModel::PowerState newState)
{
    m_energyModel->DoNotifyStateChange(newState, m_imsi);
}
...
void 
NrUeNetDevice::SetChip(NrChip chip)
{
    m_energyModel = std::make_unique<NrEnergyModel>(chip,m_energyStartTime);
    m_energyModel->m_outputDir = GetOutputDir();
}
