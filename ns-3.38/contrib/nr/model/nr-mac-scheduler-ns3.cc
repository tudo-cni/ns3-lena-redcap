        ...
        TypeId("ns3::NrMacSchedulerNs3")
            ...
            .AddAttribute("EnableHarqReTx",
                          ...
                          MakeBooleanAccessor(&NrMacSchedulerNs3::EnableHarqReTx,
                                        ...
                                        MakeBooleanChecker ())
            .AddAttribute ("NumNonOverlappingBwp",
                        "Number of Bandwitdth Parts which do not share ressources",
                        UintegerValue (0),
                        MakeUintegerAccessor (&NrMacSchedulerNs3::SetOverlappingBwp,
                                                &NrMacSchedulerNs3::GetOverlappingBwp),
                        MakeUintegerChecker<uint8_t> ())

            .AddAttribute("LegacyScheduling",
                        "Activate scheduling without the ressouce manager",
                        BooleanValue(false),
                        MakeBooleanAccessor(&NrMacSchedulerNs3::EnableLegacyScheduling,
                                            &NrMacSchedulerNs3::IsLegacySchedulingEnabled),
                        MakeBooleanChecker());
    return tid;
}
...
void
NrMacSchedulerNs3::EnableLegacyScheduling(bool legacyScheduling)
{
    m_useLegacyScheduling = legacyScheduling;
}

bool
NrMacSchedulerNs3::IsLegacySchedulingEnabled() const
{
    return m_useLegacyScheduling;
}
    ...
    const NrMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
    ...
    m_manager ->configureBwp(GetBwpId(),GetBandwidthInRbg(),GetCoresetSymbols());
    ...
    const NrMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
    ...
    itUe ->second->m_redcap = params.m_redCap;
}
    ...
    const NrMacCschedSapProvider::CschedUeReleaseReqParameters& params)
{
    ...
    m_ueMap.erase(itUe);
    m_srList.remove(params.m_rnti);
    m_msg3List.remove(params.m_rnti);
...
NrMacSchedulerNs3::DoGetMcs(uint16_t rnti)
{
    auto itUe = m_ueMap.find(rnti);

    std::tuple<uint8_t, uint8_t> mcsTuple = std::make_tuple(itUe->second->m_dlMcs.at(0),itUe->second->m_ulMcs);
    return mcsTuple;
}

void
NrMacSchedulerNs3::DoSetMcs(uint16_t rnti, std::tuple<uint8_t, uint8_t> mcsTuple)
{
    
    auto itUe = m_ueMap.find(rnti);

    itUe->second->m_dlMcs.at(0) = std::get<0>(mcsTuple);
    itUe->second->m_ulMcs = std::get<1>(mcsTuple);

}
void
NrMacSchedulerNs3::DoSetSchedulerPrachConfig(NrPhySapProvider::PrachConfig prachConfig)
{
    m_manager->reservePrachRessources(prachConfig);
}
uint64_t
    ...
    case UlCqiInfo::PUSCH: {
        ...
        for (auto it = ulAllocations.cbegin(); it != ulAllocations.cend(); /* NO INC */)
        {
            ...
            if (allocation.m_symStart == symStart)
            {
                ...
                if(itUe == m_ueMap.end())
                {
                    //Ue got removed. Skip this 
                    it = ulAllocations.erase(it);
                    continue;
                }

                if(savedMcs[UeInfoOf(*itUe)->m_rnti] != 0)
                {
                ...
                found = true;
                if(UeInfoOf(*itUe)->m_imsi == 0 && savedMcs[UeInfoOf(*itUe)->m_rnti] == 0)
                {
                    //savedMcs[UeInfoOf(*itUe)->m_rnti] = UeInfoOf(*itUe)->m_ulMcs;
                    savedMcs[UeInfoOf(*itUe)->m_rnti] = 1; //set it to 1 to save a possible mcs of 0 too
                }

                //use a fixed MCS once the MCS got calculated once
                //if(UeInfoOf(*itUe)->m_redcap==false && savedMcs[UeInfoOf(*itUe)->m_imsi] == 0 )
                // if(savedMcs[UeInfoOf(*itUe)->m_rnti] == 0 )
                // //if(GetBwpId()>=m_numOverlappingBwp && savedMcs[UeInfoOf(*itUe)->m_rnti] == 0 )
                // {
                //     savedMcs[UeInfoOf(*itUe)->m_rnti] = UeInfoOf(*itUe)->m_ulMcs;         
                // }
                // if(savedMcs[UeInfoOf(*itUe)->m_rnti] != 0)
                // {
                //     UeInfoOf(*itUe)->m_ulMcs = savedMcs[UeInfoOf(*itUe)->m_rnti];
                //     UeInfoOf(*itUe)->m_dlMcs.at(0) = savedMcs[UeInfoOf(*itUe)->m_rnti];
                // }
                it = ulAllocations.erase(it);
        ...
        //NS_ASSERT(found); //TODO_ why can "found" be false when transmitting msg3. 
                                   ...
                                   const std::string& mode) const
{
    ...
    for (const auto& ueInfo : m_ueMap)
    {
        ...
        for(auto msg3Rnti : m_msg3List) 
        {
            if(ue->m_rnti ==msg3Rnti)
            {
                //ue->transmitMsg3 = true;
            }
        }
...
uint8_t
NrMacSchedulerNs3::DoScheduleDlDataWithResManager(PointInFTPlane* spoint,
                                    uint32_t symAvail,
                                    const ActiveUeMap& activeDl,
                                    SlotAllocInfo* slotAlloc,
                                    const SfnSf& dlSfnSf) 
{
    NS_LOG_FUNCTION(this << symAvail);
    NS_ASSERT(spoint->m_rbg == 0);
    BeamSymbolMap symPerBeam = AssignDLRBG_manager(symAvail, activeDl,dlSfnSf);
    GetFirst GetBeam;
    uint8_t usedSym = 0;
    ...
    for (const auto& beam : activeDl)
    {
        ...
        std::vector<std::shared_ptr<NrMacSchedulerUeInfo>> ueInfoVec;
        for (const auto& ue : beam.second)
        {
            if (ue.first->m_dlRBG == 0)
            {
          ...
          ueInfoVec.emplace_back(ue.first);
        }

            // std::shared_ptr<DciInfoElementTdma> dci =
            //     CreateDlDci(spoint, ue.first, symPerBeam.at(GetBeam(beam)));
            std::vector<std::shared_ptr<DciInfoElementTdma>> dciVector;
        dciVector = m_manager->createDCI(ueInfoVec,GetBwpId(),dlSfnSf, m_dlAmc,LteNrTddSlotType::DL );

        
      
        for(auto dci: dciVector)
        {
            
            std::shared_ptr<ns3::NrMacSchedulerUeInfo> ueInfo;
            for(auto ueElement: ueInfoVec)
            {
                if(ueElement->m_rnti == dci->m_rnti)
                {
                    ueInfo= ueElement;
                    break;
                }   
            ...
            NS_LOG_INFO("UE " << ueInfo->m_rnti << " has " << ueInfo->m_dlRBG
                              << " RBG assigned");
            NS_ASSERT_MSG(dci->m_symStart + dci->m_numSym <= m_macSchedSapUser->GetSymbolsPerSlot(),
                          "symStart: "
                              << static_cast<uint32_t>(dci->m_symStart)
            ...
            if (!ueInfo->m_dlHarq.CanInsert())
            {
                NS_LOG_INFO("Harq Vector condition for UE " << ueInfo->m_rnti << std::endl
                                                            << ueInfo->m_dlHarq);
                NS_FATAL_ERROR("UE " << ueInfo->m_rnti << " does not have DL HARQ space");
            }

            // ueInfo->m_dlHarq.Insert(&id, harqProcess);
            // ueInfo->m_dlHarq.Get(id).m_dciElement->m_harqProcess = id;
            ...
            for (const auto& it : dci->m_tbSize)
            {
                // distribute tbsize of each stream among the LCs of the UE
                // distributedBytes size is equal to the number of LCs
                auto distributedBytes = AssignBytesToLC(ueInfo->m_dlLCG, it);
                if (bytesPerLcPerStream.size() == 0)
                {
                    bytesPerLcPerStream.resize(distributedBytes.size());
                }
            ...
            for (const auto& bytesPerLc : bytesPerLcPerStream)
            {
                ...
                for (const auto& bytesPerStream : bytesPerLc)
                {
                    if (bytesPerStream.m_bytes != 0)
                    {
                        ...
                        RlcPduInfo newRlcPdu(lcId, bytes);
                        rlcPdusInfoPerStream.push_back(newRlcPdu);
                        ueInfo->m_dlLCG.at(lcgId)->AssignedData(lcId, bytes, "DL");
                ...
                // insert rlcPduInforPerStream of a LC
                slotInfo.m_rlcPduInfo.push_back(rlcPdusInfoPerStream);
                HarqProcess& process = ueInfo->m_dlHarq.Get(dci->m_harqProcess);
                process.m_rlcPduInfo.push_back(rlcPdusInfoPerStream);
            }
...
uint8_t
NrMacSchedulerNs3::DoScheduleUlDataWithResManager(PointInFTPlane* spoint,
                                    uint32_t symAvail,
                                    const ActiveUeMap& activeUl,
                                    SlotAllocInfo* slotAlloc,
                                    const SfnSf& ulSfn) 
{
  ...
  // scheduler that has been chosen (schType_OFDMA and FlexOFDMA are programmed inside this function).
   ...
   // BeamSymbolMap symPerBeam = AssignULRBG(symAvail, activeUl);
    ...
    //instead of assigning ULRBG we inform the ressource manager for UL Scheduling
   BeamSymbolMap symPerBeam = AssignULRBG_manager(symAvail, activeUl,ulSfn);
    ...
    for (const auto& beam : activeUl)
    {
        ...
        std::vector<std::shared_ptr<DciInfoElementTdma>> dciVector;
        dciVector = m_manager->createDCI(ueInfoVec,GetBwpId(),ulSfn, m_ulAmc, LteNrTddSlotType::UL);
        ...
        for(auto dci: dciVector)
        {
            ...
            if (symbStartDci.insert(dci->m_symStart).second)
            {
             ...
             if (!ueInfo->m_ulHarq.CanInsert())
            {
                NS_LOG_INFO("Harq Vector condition for UE " << ueInfo->m_rnti << std::endl
                                                            << ueInfo->m_ulHarq);
                NS_FATAL_ERROR("UE " << ueInfo->m_rnti << " does not have UL HARQ space");
            }

            HarqProcess harqProcess(true, HarqProcess::WAITING_FEEDBACK, 0, dci);
            //uint8_t id;
            // ueInfo->m_ulHarq.Insert(&id, harqProcess);

            // ueInfo->m_ulHarq.Get(id).m_dciElement->m_harqProcess = id;

            VarTtiAllocInfo slotInfo(dci);
            // if(ueInfo->transmitMsg3)
            // {
            //     slotInfo.m_isMsg3 =true;
            // }
            ...
            auto distributedBytes = AssignBytesToLC(ueInfo->m_ulLCG, dci->m_tbSize.at(0));
            bool assignedToLC = false;
            for (const auto& byteDistribution : distributedBytes)
            {
                assignedToLC = true;
             
                ueInfo->m_ulLCG.at(byteDistribution.m_lcg)
                    ->AssignedData(byteDistribution.m_lcId, byteDistribution.m_bytes, "UL");
                NS_LOG_DEBUG("UL LCG " << static_cast<uint32_t>(byteDistribution.m_lcg)
                                       << " assigned bytes " << byteDistribution.m_bytes
                                       << " to LCID "
...
NrMacSchedulerNs3::DoScheduleUlSr(PointInFTPlane* spoint, const std::list<uint16_t>& rntiList) const
{
    ...
    for (const auto& v : rntiList)
    {
        for (auto& ulLcg : NrMacSchedulerUeInfo::GetUlLCG(m_ueMap.at(v)))
        {
            ...
            if(ulLcg.second->GetTotalSize() != 0)
            {
                ulLcg.second->UpdateInfo(ulLcg.second->GetTotalSize()+12);
            }
            else{
...
void
NrMacSchedulerNs3::DoScheduleMsg3( std::list<uint16_t>& rntiList,SlotAllocInfo* slotAlloc ,const SfnSf& ulSfn) const
{
    NS_LOG_FUNCTION(this);
    std::vector<std::vector<int>> slotRessources = m_manager->getSlotRessources(GetBwpId(),ulSfn,LteNrTddSlotType::UL);
    
    for( auto& rnti: rntiList)
    {
        bool found = false;
        uint32_t tbs;
        ...
        {
            for(size_t rb =0; rb< 51;rb++)
            {
                
                if(slotRessources[i][rb]== rnti)
                {
                    found= true;
                    StartSymbol = i;
                    startRb = rb;
                    uint count = 0;
                    while(slotRessources[i][rb+count] == rnti)
                    {
                        NumRB++;
                        count++;
                    }
                    count = 0;
                    while(slotRessources[i+count][rb] == rnti)
                    {
                       symNum++;
                       count++; 
                    }
        ...
        if(found)
        {
            //std::cout<<"msg3 for ue "<<ue<<std::endl;
            rbgmask = m_manager->createRBGmask(bwpIndex,NumRB,startRb,symNum,StartSymbol);

            DciInfoElementTdma::DciFormat dciFormat;

            dciFormat =  DciInfoElementTdma::DciFormat::UL;
            mcs = {0}; //Should the MCS always be 0 for msg3?
            tbs = m_ulAmc->CalculateTbSize (0,NumRB*symNum);
    
            std::vector<uint32_t> m_tbs = {tbs};
            ...
            std::cout<<"Warum sind keine msg3 ressourcen vorhanden"<<std::endl;
        }
...
void
NrMacSchedulerNs3::ScheduleDLWithResManager(const NrMacSchedSapProvider::SchedDlTriggerReqParameters& params,
                    const std::vector<DlHarqInfo>& dlHarqFeedback)
{
    NS_LOG_FUNCTION(this);
    NS_LOG_FUNCTION(this);
    ...
    for (const auto& rachReq : m_rachList)
    {
        BuildRarListElement_s newRar;
        newRar.m_rnti = rachReq.m_rnti;
        newRar.raRnti = rachReq.raRnti;
         // Calculate the RBs to schedule for a UE
        uint32_t bytesToSend = 10; 
        if(rachReq.schedSdtRes)
        {
            bytesToSend = bytesToSend+125;
        }
        uint tbs = 0;
        uint32_t rbPacket =1;

        //find the ue with the same imsi for an accurate mcs
        uint8_t mcs = savedMcs[rachReq.m_rnti]; //changed to m_rnti from m_imsi
        DoSetMcs(rachReq.m_rnti,std::make_tuple(mcs,mcs));
        
        //fill ueMap with imsi
        for (auto element : m_ueMap)
        {
         
            if(element.second->m_rnti == rachReq.m_rnti)
            {
                element.second->m_imsi = rachReq.m_imsi;
                break;
            }
        ...
        while(tbs <bytesToSend+5) //+5 to be able to transmit a short BSR which is included in the Message
             {
                tbs = m_ulAmc->CalculateTbSize (mcs,rbPacket);
                rbPacket++;
            }
        RessourceSet* ulRes = new RessourceSet(); 
        ulRes = m_manager ->scheduleData(GetBwpId(),newRar.m_rnti,rbPacket, LteNrTddSlotType::UL,true,params.m_snfSf,ulRes); 
        
        if(ulRes->NumberRB!=0 &&ulRes->NumberSymbols!=0 )
        {
            newRar.m_grant.m_rnti = newRar.m_rnti;
            newRar.m_grant.m_rbStart = ulRes->StartRB;
            newRar.m_grant.m_rbLen = ulRes->NumberRB;
            newRar.m_grant.m_tbSize = tbs; //TODO_! akkurat definieren
            newRar.m_grant.m_mcs = mcs;
            newRar.m_grant.m_Framenumber = ulRes->Framenumber;
            newRar.m_grant.m_Subframenumber = ulRes->Subframenumber;
            newRar.m_grant.m_Slotnumber = ulRes->Slotnumber;
            newRar.m_grant.m_StartSymbol = ulRes->StartSymbol;
            newRar.m_grant.m_NumberSymbols = ulRes->NumberSymbols;
            newRar.m_grant.m_rbgBitmask = ulRes->rbgmask;
            dlSlot.m_buildRarList.push_back(newRar);
            

            //SlotAllocation muss an den Phy übergeben werden
            // std::vector<uint8_t> ndi = {1}; 
            // std::vector<uint8_t> rv = {0};
            // std::vector<uint8_t> Msg3mcs;
            // std::vector<uint32_t> msg3Tbs = {tbs};
            // Msg3mcs = {mcs};

            // auto Msg3Dci = std::make_shared<DciInfoElementTdma>(newRar.m_grant.m_rnti,
            //                                             DciInfoElementTdma::UL,
            //                                             newRar.m_grant.m_StartSymbol,
            //                                             newRar.m_grant.m_NumberSymbols,
            //                                             Msg3mcs,
            //                                             msg3Tbs,
            //                                             ndi,
            //                                             rv, 
            ...
            //                                             0,
            //                                             0,//TODO?
            //                                             newRar.m_grant.m_rbgBitmask,
            //                                             GetTpc());
            // VarTtiAllocInfo varTtiInfo(Msg3Dci);
            // varTtiInfo.m_isMsg3 = true;
            // SfnSf msg3SnfSF = SfnSf(newRar.m_grant.m_Framenumber,newRar.m_grant.m_Subframenumber,newRar.m_grant.m_Slotnumber,1); //numerology hardcoded


            // SlotAllocInfo slotAllocInfo = SlotAllocInfo(msg3SnfSF);
            ...
            std::cout<<"failed to get ressources for msg3" << std::endl;
        }
        delete ulRes;
    ...
    for (auto it = activeDlUe.begin(); it != activeDlUe.end();++it)
    {
        ...
        for (auto ueIt = ueInfos.begin(); ueIt != ueInfos.end();)
        {
            GetFirst GetUeInfoPtr;            
            if(m_manager->markViablePdcchRessource(false,params.m_snfSf,GetUeInfoPtr(*ueIt)->m_rnti, GetBwpId()))
            {
                NS_LOG_DEBUG("Searchspace for RNTI "<<GetUeInfoPtr(*ueIt)->m_rnti<< "available");  
                ++ueIt;          
            }
            else{
                NS_LOG_DEBUG("Searchspace RNTI "<<GetUeInfoPtr(*ueIt)->m_rnti<< "from active UE list -> no available SearchSpace"); 
                ueInfos.erase(ueIt);
                //std::cout<<"no viable searchspace for rnti: "<< GetUeInfoPtr(*ueIt)->m_rnti<< std::endl;
                continue;
            } 
    ...
    //remove ue which got removed by inactivity
    for (auto it = activeDlUe.begin(); it != activeDlUe.end();++it)
    {
        auto& ueInfos = GetUeInfoList(*it);
        for (auto ueIt = ueInfos.begin(); ueIt != ueInfos.end();)
        {
            GetFirst GetUeInfoPtr;
            if(m_ueMap.find(GetUeInfoPtr(*ueIt)->m_rnti) == m_ueMap.end())
            {
                ueInfos.erase(ueIt);
                //std::cout<<"removed ue "<<GetUeInfoPtr(*ueIt)->m_rnti<< " from scheduling"<<std::endl;
                continue;
            }
    ...
    DoScheduleDlWithResManager(dlHarqFeedback,
                 activeDlHarq,
                 &activeDlUe,
                 params.m_snfSf,
                 ulAllocations,
...
void
NrMacSchedulerNs3::ScheduleULWithResManager(const NrMacSchedSapProvider::SchedUlTriggerReqParameters& params,
                                    const std::vector<UlHarqInfo>& ulHarqFeedback)
{
    NS_LOG_FUNCTION(this);
    NS_LOG_INFO("Scheduling invoked for slot " << params.m_snfSf);
    ...
    // Doing UL for slot ulSlot
    DoScheduleUlWithResManager(ulHarqFeedback, params.m_snfSf, &ulSlot.m_slotAllocInfo, params.m_slotType);
...
uint8_t
NrMacSchedulerNs3::DoScheduleUlWithResManager(const std::vector<UlHarqInfo>& ulHarqFeedback,
                                const SfnSf& ulSfn,
                                SlotAllocInfo* allocInfo,
                                LteNrTddSlotType type)
{
  ...
  uint8_t ulSymAvail = dataSymPerSlot;
  uint8_t StartSym = m_macSchedSapUser->GetSymbolsPerSlot () - m_ulCtrlSymbols;
  ...
  uint8_t ulSymAvail = dataSymPerSlot;*/
    ...
    if (ulSymAvail > 0 && m_msg3List.size() > 0)
    {
        DoScheduleMsg3(m_msg3List,allocInfo,ulSfn);
    }
    ...
    for (const auto& alloc : allocInfo->m_varTtiAllocInfo)
    {
        for (auto it = activeUlUe.begin(); it != activeUlUe.end(); /* no incr */)
        {
            ...
            for (auto ueIt = ueInfos.begin(); ueIt != ueInfos.end(); /* no incr */)
            {
                ...
                if(m_fixedMcsUl)
                {
                    ueIt->first->m_ulMcs= {GetStartMcsUl()};
    ...
    {
        uint8_t usedUl =
            DoScheduleUlDataWithResManager(&ulAssignationStartPoint, ulSymAvail, activeUlUe, allocInfo,ulSfn);
        NS_LOG_INFO("For the slot " << ulSfn << " reserved " << static_cast<uint32_t>(usedUl)
                                    << " symbols for UL data tx");
        ulSymAvail -= usedUl;
    }
    ...
    //This assert doesnt work properly because we defined an overlapping bwp which uses the same ressources.
    // NS_ASSERT_MSG((dataSymPerSlot + m_ulCtrlSymbols) - ulSymAvail == totUlSym,
    //               "UL symbols available: "
    //                   << static_cast<uint32_t>(dataSymPerSlot + m_ulCtrlSymbols)
    //                   << " UL symbols available at end of sched: "
...
uint8_t
NrMacSchedulerNs3::GetCoresetSymbols() const
{
    return m_macSchedSapUser->GetCoresetSymbols();
}
                                ...
                                SlotAllocInfo* allocInfo)
{
    ...
    {
        uint8_t usedDl =
            DoScheduleDlData(&dlAssignationStartPoint, dlSymAvail, *activeDlUe, allocInfo,dlSfnSf);
...
uint8_t
NrMacSchedulerNs3::DoScheduleDlWithResManager(const std::vector<DlHarqInfo>& dlHarqFeedback,
                                const ActiveHarqMap& activeDlHarq,
                                ActiveUeMap* activeDlUe,
                                const SfnSf& dlSfnSf,
                                const SlotElem& ulAllocations,
                                     ...
                                     LteNrTddSlotType type)
{
  ...
  uint8_t dlSymAvail = dataSymPerSlot - ulAllocations.m_totUlSym;
    ...
    for (const auto& alloc : allocInfo->m_varTtiAllocInfo)
    {
        ...
        {   
            ...
            for (auto ueIt = ueInfos.begin(); ueIt != ueInfos.end(); /* no incr */)
            {
                ...
                if(m_fixedMcsDl)
                {
                    ueIt->first->m_dlMcs= {GetStartMcsDl()};
                    for(uint mimoLayer = 1; mimoLayer< ueIt->first->m_dlTbSize.size();++mimoLayer)
                    {
                        ueIt->first->m_dlMcs.push_back(GetStartMcsDl());
                    }
                }
    ...
    NS_ASSERT(dlAssignationStartPoint.m_rbg == 0);
    if (dlSymAvail > 0 && activeDlUe->size() > 0 && type != LteNrTddSlotType::S)
    {
        uint8_t usedDl =
            DoScheduleDlDataWithResManager(&dlAssignationStartPoint, dlSymAvail, *activeDlUe, allocInfo, dlSfnSf);
        NS_ASSERT(dlSymAvail >= usedDl);
        dlSymAvail -= usedDl;
    }
    ...
    const NrMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    if (params.m_dlHarqInfoList.size() > 0 || m_dlHarqToRetransmit.size() > 0)
    {
        ...
        //         std::cout<<"test process inactive"<<std::endl;
        ...
        //                 std::cout<<"test duplicate feedback"<<std::endl;
        //                 it = dlHarqFeedback.erase(it); /* INC */
        //             }
        //         }
        //     }
    ...
    if(IsLegacySchedulingEnabled())
    {
        ScheduleDl(params, dlHarqFeedback);
    }
    else{
        ScheduleDLWithResManager(params,dlHarqFeedback);
    }
    ...
    const NrMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    if (params.m_ulHarqInfoList.size() > 0 || m_ulHarqToRetransmit.size() > 0)
    {
        ...
        //      if(itUe == m_ueMap.end())
        //     {
        //         //UE not longer active. Skip Scheduling
        //         std::cout<<"test ul inactive"<<std::endl;
        //         it = ulHarqFeedback.erase(it);
        //         continue;
            
        //     }
        //     auto& ueInfo = itUe->second;
        ...
        //                                        << static_cast<uint32_t>(it->m_harqProcessId)
        //                                        << " ignored because process is INACTIVE");
        //         std::cout<<"test ulprocess inactive"<<std::endl;
        //         it = ulHarqFeedback.erase(it);
        //     }
        //     else
        //     {
    ...
    if(IsLegacySchedulingEnabled())
    {
        ...
        ScheduleULWithResManager(params,ulHarqFeedback);
    }
}
    ...
    const NrMacSchedSapProvider::SchedUlSrInfoReqParameters& params)
{
    ...
    for (const auto& ue : params.m_srList)
    {
        ...
        if(m_ueMap.find(ue)==m_ueMap.end())
        {
            NS_LOG_INFO("UE is not included in ueMap. Skipping SR");
            break;
        }
    ...
    //NS_ASSERT(m_srList.size() >= params.m_srList.size()); //SR can be received after removing ue. In that case we should not add it to the list
}

void
NrMacSchedulerNs3::DoSchedUlMsg3InfoReq(
    const NrMacSchedSapProvider::SchedUlMsg3InfoReqParameters& params)
{
    NS_LOG_FUNCTION(this);

    // Merge RNTI in our current list
    for (const auto& ue : params.m_msg3List)
    {
        NS_LOG_INFO("UE " << ue << " notified to transmit Msg3");

        auto it = std::find(m_msg3List.begin(), m_msg3List.end(), ue);
        if (it == m_msg3List.end())
        {
            m_msg3List.push_back(ue);
        }
    }
    NS_ASSERT(m_msg3List.size() >= params.m_msg3List.size());
}

void
NrMacSchedulerNs3::SetOverlappingBwp (uint8_t bwpNum)
{
  m_numOverlappingBwp = bwpNum;
}

uint8_t
NrMacSchedulerNs3::GetOverlappingBwp () const
{
  return m_numOverlappingBwp;
}

void
NrMacSchedulerNs3::SetManager(NrMacSchedulerRessourceManager*  &schedmanager)
{

    m_manager = schedmanager;
