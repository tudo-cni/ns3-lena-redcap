...
#define MIN_NO_CC 1
#define MAX_NO_CC 10 //had to be increased to support more BWP (a bwp is realised as cc)
 ...
 * single UE.
 */
class UeManagerNr : public UeManager
{
    /// allow LtePdcpSpecificLtePdcpSapUser<UeManager> class friend access
    friend class LtePdcpSpecificLtePdcpSapUser<UeManagerNr>;
  ...
  public:
     ...
     *
     * \param rrc pointer to the NrGnbRrc holding this UeManager
     * \param rnti RNTI of the UE
     * \param s initial state of the UeManager
     * \param componentCarrierId primary component carrier ID
     */
    UeManagerNr(Ptr<NrGnbRrc> rrc, uint16_t rnti, State s, uint8_t componentCarrierId);

    ~UeManagerNr() override;
    bool inRACH{false};
  ...
  public:
    ...
    /**
     * schedule an RRC Reconfiguration procedure with the UE
     *
     */
    void ScheduleRrcReconfiguration();
     ...
     */
    void RecvRrcConnectionRequest(NrRrcSap::NrRrcConnectionRequest msg);

    void RecvRrcResumeRequest(NrRrcSap::RrcResumeRequest msg);
    /**
    ...
    void RecvRrcResumeComplete(NrRrcSap::RrcResumeComplete msg);
    /**
    ...
    void BwpSwitchCompleted();

    void RefreshDataInactivityTimeout();

    bool GetSdtConfigured();
  ...
  private:
    ...
    /**
     *
     * \return an RrcnReconfiguration struct built based on the
     * current configuration
     */
    NrRrcSap::RrcReconfiguration BuildRrcReconfiguration();
    ...
    /**
     *
     * \return a RadioBearerConfig struct built based on the
     * current configuration
     */

    LteRrcSap::RadioResourceConfigDedicated BuildRadioResourceConfigDedicated ();

    NrRrcSap::RadioBearerConfig BuildRadioBearerConfiguration();

    NrRrcSap::SpCellConfig BuildSpCellconfig();

    NrRrcSap::SuspendConfig BuildSuspendConfig();

    uint16_t CalculateLocAndBw(uint8_t bwpID);
...
class NrGnbRrc : public Object
{
    ...
    /// allow MemberLteHandoverManagementSapUser<NrGnbRrc> class friend access
    friend class MemberLteHandoverManagementSapUser<NrGnbRrc>;
    /// allow MemberLteAnrSapUser<NrGnbRrc> class friend access
    friend class MemberLteAnrSapUser<NrGnbRrc>;
    /// allow MemberLteFfrRrcSapUser<NrGnbRrc> class friend access
    friend class MemberLteFfrRrcSapUser<NrGnbRrc>;
    /// allow MemberNrGnbRrcSapProvider<NrGnbRrc> class friend access
    friend class MemberNrGnbRrcSapProvider<NrGnbRrc>;
    /// allow MemberNrGnbRrcSapProvider<NrGnbRrc> class friend access
    friend class MemberEpcEnbS1SapUser<NrGnbRrc>;
    /// allow MemberEpcEnbS1SapUser<NrGnbRrc> class friend access
    friend class EpcX2SpecificEpcX2SapUser<NrGnbRrc>;
    /// allow UeManager class friend access
    friend class UeManagerNr;
    /// allow  MemberLteCcmRrcSapUser<NrGnbRrc> class friend access
    friend class MemberLteCcmRrcSapUser<NrGnbRrc>;
  ...
  public:
     ...
     */
    Ptr<UeManagerNr> GetUeManager(uint16_t rnti);
    ...
    void SetSearchSpaceCreatedCallback(Callback<void,uint16_t,uint16_t,uint16_t,uint16_t> sscb);

    void SetUpdateRntiMapCallback(Callback<void,uint64_t,uint16_t> urmc);
    ...
    void DataInactivityTimeout(uint16_t rnti);
  ...
  public: //TODO warum geht priva
     ...
     *
     * \param rnti the RNTI
     * \param msg the NrRrcSap::NrRrcConnectionRequest
     */
    void DoRecvRrcConnectionRequest(uint16_t rnti, NrRrcSap::NrRrcConnectionRequest msg);

    void DoRecvRrcResumeRequest(uint16_t rnti, NrRrcSap::RrcResumeRequest msg);
    /**
    ...
    void DoRecvRrcResumeComplete(uint16_t rnti,
    ...
    void DoUpdateGnbBwpMap(uint16_t rnti, uint8_t bwpID);
    ...
    void DoNotifySwitchedBwp(uint8_t bwpID);
     ...
     */
    uint16_t AddUe(UeManagerNr::State state, uint8_t componentCarrierId);
  ...
  public:
    ...
    void SetSdtUsage(bool state);

    bool GetSdtUsage() const;
  ...
  protected:
    ...
    Callback<void,uint16_t,uint16_t,uint16_t,uint16_t> m_searchSpaceCreatedCallback; 
    Callback<void,uint64_t,uint16_t> m_updateRntiMapCallback; 
     ...
     */
    std::map<uint16_t, Ptr<UeManagerNr>> m_ueMap;


    //maps for configuring viable Bwp for different types of UE
    std::string m_embbBwp;
    std::string m_redcapBwp;
    
    void  SetBwpForEmBB(const std::string& bwp);
    std::string GetBwpForEmBB() const;

    void  SetBwpForRedCap(const std::string& bwp);
    std::string GetBwpForRedCap() const;
    //std::map<uint16_t, uint16_t> m_redCapBwp;
    ...
    uint8_t m_prachConfiguraionIndex;
    ...
    /**
     * The `Reconfiguration` trace source. Fired upon RRC connection
     * reconfiguration. Exporting IMSI, cell ID, and RNTI.
     */
    TracedCallback<uint64_t, uint16_t, uint16_t> m_ReconfigurationTrace;
    /**
    ...
    /**
     * The `NotifyRelease` trace source. Fired when an UE leaves the eNB.
     * Exporting IMSI, cell ID, RNTI.
     *
     */
    TracedCallback<uint64_t, uint16_t, uint16_t> m_ReleaseTrace;
    /**
    ...
    std::vector<uint8_t> redCapDedicatedBwps; //bwps used for RedCap devices
    std::vector<uint8_t> eMBB_DedicatedBwps; //bwps used for eMBB devices
    bool m_useSdt{false};
    bool m_droneUsage;
    bool bwpConfigured{false};
