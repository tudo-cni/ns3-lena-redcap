...
class NrMacSchedulerNs3 : public NrMacScheduler
{
  public:
    ...
    void DoSchedUlMsg3InfoReq(
        const NrMacSchedSapProvider::SchedUlMsg3InfoReqParameters& params) override;
    ...
    void DoSetSchedulerPrachConfig(NrPhySapProvider::PrachConfig prachConfig);
    /**
     ...
     */
    typedef std::pair<UePtr, uint32_t> 
    UePtrAndBufferReq;
    /**
    ...
    void EnableLegacyScheduling(bool legacyScheduling);

    bool IsLegacySchedulingEnabled() const;

    void SetOverlappingBwp (uint8_t numBwp);
    uint8_t GetOverlappingBwp () const;
  

  void SetManager(NrMacSchedulerRessourceManager* &schedManager);
  ...
  protected:
    ...
    virtual BeamSymbolMap AssignDLRBG_manager(uint32_t symAvail, const ActiveUeMap& activeDl, const SfnSf& dlSfnSf) = 0;
    ...
    virtual BeamSymbolMap AssignULRBG_manager(uint32_t symAvail, const ActiveUeMap& activeUl, const SfnSf& ulSfn) = 0;
  ...
  private:
    ...
    void ScheduleDLWithResManager(const NrMacSchedSapProvider::SchedDlTriggerReqParameters& params,
                    const std::vector<DlHarqInfo>& dlHarqInfo);

    void ScheduleUl(const NrMacSchedSapProvider::SchedUlTriggerReqParameters& params,
                    const std::vector<UlHarqInfo>& ulHarqInfo);

    void ScheduleULWithResManager(const NrMacSchedSapProvider::SchedUlTriggerReqParameters& params,
                    const std::vector<UlHarqInfo>& ulHarqInfo);
    ...
    uint8_t DoScheduleDlData(PointInFTPlane* spoint,
                             ...
                             const SfnSf& dlSfnSf) ;
    uint8_t DoScheduleDlDataWithResManager(PointInFTPlane* spoint,
                                uint32_t symAvail,
                                const ActiveUeMap& activeDl,
                                SlotAllocInfo* slotAlloc,
                                const SfnSf& dlSfnSf) ;
    uint8_t DoScheduleUlData(PointInFTPlane* spoint,
                             ...
                             SlotAllocInfo* slotAlloc);
    uint8_t DoScheduleUlDataWithResManager(PointInFTPlane* spoint,
                                uint32_t symAvail,
                                const ActiveUeMap& activeUl,
                                SlotAllocInfo* slotAlloc, const SfnSf& ulSfn);
    ...
    uint8_t DoScheduleDlWithResManager (const std::vector <DlHarqInfo> &dlHarqFeedback, const ActiveHarqMap &activeDlHarq,
                        ActiveUeMap *activeDlUe, const SfnSf &dlSfnSf,
                        const SlotElem &ulAllocations, SlotAllocInfo *allocInfo,
    ...
    uint8_t DoScheduleUlWithResManager (const std::vector <UlHarqInfo> &ulHarqFeedback, const SfnSf &ulSfn,
                            SlotAllocInfo *allocInfo, LteNrTddSlotType type);
  ...
  void DoScheduleMsg3(std::list<uint16_t> &rntiList,SlotAllocInfo* allocInfo, const SfnSf &ulSfn) const;
  ...
  protected:
    ...
    // pointer to the ressource manager
    NrMacSchedulerRessourceManager* m_manager;
  private:
    ...
    std::map<uint16_t, uint8_t> savedMcs; //<imsi,mcs>, save mcs for future accurate msg3 transmissions
    ...
    bool m_useLegacyScheduling{false};
    uint8_t m_numOverlappingBwp {0}; 
