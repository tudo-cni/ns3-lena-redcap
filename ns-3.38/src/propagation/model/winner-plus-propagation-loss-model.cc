...
void
WinnerPlusPropagationLossModel::SetChannelConditionModel(Ptr<ChannelConditionModel> model)
{
...
WinnerPlusPropagationLossModel::GetChannelConditionModel() const
{
    NS_LOG_FUNCTION(this);
...
WinnerPlusPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  //Winner2 model
  else if (m_environment == RMaEnvironment){
    if (m_los == true){
        loss = 40.0 * std::log10(dist) + 10.5 - 18.5*std::log10(hbs) -18.5*std::log10(hms) + 1.5*std::log10(fghz/5.0);
    }
    else if (m_los == false){
        loss = 25.1 * std::log10(dist) + 55.4 - 0.13*(hbs-25)*std::log10(dist/100) -0.9*(hms-1.5) + 21.3*std::log10(fghz/5.0);
    }
  }
