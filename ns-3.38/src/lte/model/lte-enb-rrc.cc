...
class EnbRrcMemberLteEnbCmacSapUser : public LteEnbCmacSapUser
{
  public:
    ...
    bool isRedCapUe(uint16_t rnti) override;
    void TriggerRelease(uint16_t rnti) override;
    void NotifyDataActivity(uint16_t rnti) override;
    bool IsSdtUsable(uint16_t rnti) override;
...
{
    NS_FATAL_ERROR("Not implemented for LTE module");
}

bool
EnbRrcMemberLteEnbCmacSapUser::IsSdtUsable(uint16_t rnti)
{
    NS_FATAL_ERROR("Should not be called for LTE");
}
...
UeManager::UeManager()
{
    ...
    //This contructor is called from UeManagerNr. NrGnbRRc and many other modules have references to LteEnbRrc and therefore
    //i made NrGnbRr inherit from LteEnbRrc. 
}
...
LteEnbRrc::ConfigureCell(std::map<uint8_t, Ptr<ComponentCarrierBaseStation>> ccPhyConf)
{
    ...
    for (const auto& it : ccPhyConf)
    {
        ...
        sib1.siSchedulingInfo.sIRequestConfig.rachOccasionsSI.rachConfigSI.prachConfigurationIndex = 123;
        sib1.siSchedulingInfo.sIRequestConfig.rachOccasionsSI.rachConfigSI.choice = LteRrcSap::RachConfigSI::msg1FDM::ONE; //msg1FDM
        ...
        //sib1.siSchedulingInfo.sIRequestConfig.rachOccasionsSI.ssbPerRach = LteRrcSap::RachOccasionsSI::ssbPerRACHOccasion::ONE; //ssbPerRACHOccasion
...
void
LteEnbRrc::DoRecvRrcResumeRequest(uint16_t rnti, NrRrcSap::RrcResumeRequest msg)
{
    NS_FATAL_ERROR("Not supported for LTE");
}
...
void
LteEnbRrc::DoNotifySwitchedBwp(uint8_t bwpID)
{
   NS_FATAL_ERROR("LteEnbRrc shouldnt use this method");
}
...
bool 
LteEnbRrc::isRedCapUe(uint16_t rnti)
{
    //NS_FATAL_ERROR("LTE doesnt support RedCap UEs");
    return false;
}
