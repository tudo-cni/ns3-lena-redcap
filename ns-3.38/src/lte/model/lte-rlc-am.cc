...
LteRlcAm::DoTransmitPdcpPdu(Ptr<Packet> p)
{
    ...
    else
    {
        ...
        std::cout<<"packet droped for rnti:"<< m_rnti<<std::endl;
        m_txDropTrace(p);
    }
...
void
LteRlcAm::DoSendMsg3(Ptr<Packet> p, uint16_t grantBytes)
{
    NS_FATAL_ERROR("Msg3 shouldnt be transmitted over LteRlcAm.");
}
...
LteRlcAm::DoNotifyTxOpportunity(LteMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    if (m_statusPduRequested && !m_statusProhibitTimer.IsRunning())
    {
        if (txOpParams.bytes < m_statusPduBufferSize)
        {
            ...
            NS_ASSERT_MSG(false,
                          "TxOpportunity (size = "
                              ...
                              << m_statusPduBufferSize << ")\n"
                              << "Your MAC scheduler has assigned too few resource blocks.");
            return;
        }
        ...
        //std::cout << "Sending ACK Sn: " << uint(sn.GetValue())<<"for rnti "<<m_rnti<<" at " <<Simulator::Now() <<std::endl;
        // std::string logfile_path = "Results/RLC_AM.log";
        // std::ofstream logfile;
        // logfile.open (logfile_path, std::ios_base::app );
        // logfile <<"Sending ACK Sn: " << uint(sn.GetValue())<<"for rnti "<<m_rnti<<" at " <<Simulator::Now() << "\r\n";
        // logfile.close ();
    ...
    else if (m_retxBufferSize > 0)
    {
        ...
        //std::cout<<"Sending data from Retransmission Buffer for "<<m_rnti<<std::endl;
        ...
        for (sn = m_vtA; sn < m_vtS; sn++)
        {
            ...
            if (m_retxBuffer.at(seqNumberValue).m_pdu)
            {
                ...
                else
                {
                    ...
                    //std::cout<<"zu wenig Bytes für retransmission zugeteilt für Rnti: "<< m_rnti << std::endl;
                    // std::string logfile_path = "Results/RLC_AM.log";
                    // std::ofstream logfile;
                    // logfile.open (logfile_path, std::ios_base::app );
                    // logfile <<"zu wenig Bytes für retransmission zugeteilt für Rnti: "<< m_rnti << "\r\n";
                    // logfile.close ();
                    return;
                }
            }
    ...
    {
        //std::cout<<"Keine Daten verfügbar für " <<m_rnti<<std::endl;
    ...
    while (firstSegment && (firstSegment->GetSize() > 0) && (nextSegmentSize > 0))
    {
            ...
            (firstSegment->GetSize() > 2047))
        {
            ...
            if(m_rnti == 3)
            {
                //std::cout<<"first segment:  "<< uint(firstSegment->GetSize())<< " next segment " << nextSegmentSize<<std::endl;
            }
            ...
            if (firstSegment->GetSize() > 0)
            {
                if(m_rnti == 3)
                {
                    //std::cout<<"Txon buffer: Give back the remaining segment"<<std::endl;
                }
        ...
        else if ((nextSegmentSize - firstSegment->GetSize() <= 2) || m_txonBuffer.empty())
        {
            NS_LOG_LOGIC(
             ...
             if(m_rnti == 3)
            {
                //std::cout<<"2:first segment:  "<< uint(firstSegment->GetSize())<< " next segment " << nextSegmentSize<<std::endl;
            }
    ...
    if(m_rnti == 3)
    {
        //std::cout<<m_vtS.GetValue() << std::endl;
    }
...
LteRlcAm::DoReceivePdu(LteMacSapUser::ReceivePduParameters rxPduParams)
{
    ...
    if(m_rnti == 3)
    {
        //std::cout<<"DoReceivePdu: "<< uint( rxPduParams.p->GetSize())<< std::endl;
    }
    ...
    else if (rlcAmHeader.IsControlPdu())
    {
        ...
        for (sn = m_vtA; sn < ackSn && sn < m_vtS; sn++)
        {
            ...
            {
                std::cout<<"NACK sn "<<sn << " for rnti: "<<m_rnti<<std::endl;
                // std::string logfile_path = "Results/RLC_AM.log";
                // std::ofstream logfile;
                // logfile.open (logfile_path, std::ios_base::app );
                // logfile <<"NACK sn "<<sn << " for rnti: "<<m_rnti<< "\r\n";
                // logfile.close ();
            ...
            else
            {
                ...
                if(m_rnti == 3)
                {
                    //std::cout<<"sn " << sn << " is ACKed for rnti: "<< m_rnti<<" at "<< Simulator::Now()<<std::endl;
                ...
                // std::ofstream logfile;
                // logfile.open (logfile_path, std::ios_base::app );
                // logfile << sn << " is ACKed for rnti: "<< m_rnti<<" at "<< Simulator::Now()<< "\r\n";
                // logfile.close ();
...
LteRlcAm::DoReportBufferStatus()
{
    ...
    if(m_rnti == 3)
    {
        //std::cout<<"LteRlcAm::DoReportBufferStatus , "<<"txonBufferSize = " << m_txonBufferSize<< ",retxBufferSize = " << m_retxBufferSize<< ",txedBufferSize = " << m_txedBufferSize<< std::endl;
    }
...
LteRlcAm::ExpireReorderingTimer()
{
    ...
    std::cout<<"ExpireReorderingTimer"<<std::endl;
...
LteRlcAm::ExpirePollRetransmitTimer()
{
    std::cout<<"ExpirePollRetransmitTimer for "<<m_rnti<<std::endl;
    NS_LOG_FUNCTION(this);
...
LteRlcAm::ExpireRbsTimer()
{
    ...
    if (m_txonBufferSize + m_txedBufferSize + m_retxBufferSize > 0)
    {
         if(m_rnti == 3)
        {
            //std::cout<<"starting bsr"<<std::endl;
        }
        DoReportBufferStatus();
        m_rbsTimer = Simulator::Schedule(m_rbsTimerValue, &LteRlcAm::ExpireRbsTimer, this);
    }
